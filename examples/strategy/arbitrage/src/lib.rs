...
pub fn get_profit_at_x(
    a1: u128, b1: u128, a2: u128, b2: u128, x: u128,
) -> i128 {
...
pub fn get_max_profit_point(
    a1: u128, b1: u128, a2: u128, b2: u128,
) -> (i128, i128) {
    // TODO: prevent overflow
    let optimal_x = (Isqrt::isqrt(a1*b1).checked_mul(Isqrt::isqrt(a2*b2)).unwrap() - b1*a2) / (a1+a2);
    (optimal_x as i128, get_profit_at_x(a1, b1, a2, b2, optimal_x))
}
...
pub fn astro_transfer(src_plane: String, dst_plane: String, denom: String, amount: u128) -> Option<FISInstruction> {
    None
}
...
pub fn query(_deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    ...
    pools.iter().for_each(|p| {
        ...
        let (x, y) = get_max_profit_point(src_pool.a.u128(), src_pool.b.u128(), p.a.u128(), p.b.u128());
        if y > optimal_y {
            optimal_y = y;
            optimal_x = x;
            selected_pool = p;
    ...
    let expected_output = if execute_amount == optimal_x as u128 {
        ...
        get_profit_at_x(src_pool.a.u128(), src_pool.b.u128(), selected_pool.a.u128(), selected_pool.b.u128(), execute_amount)
    };

    // use the amount to buy
    let instructions = vec![
        swap(msg.clone().source_plane, src_pool.id.clone(), execute_amount, true).unwrap(),
        astro_transfer(msg.source_plane.clone(), selected_pool.plane.clone(), msg.denom, expected_output as u128).unwrap(),
        swap(selected_pool.plane.to_string(), selected_pool.id.clone(), expected_output as u128, false).unwrap(), // zero for one? we don't know
    ];
